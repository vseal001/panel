...
get_node_url(){
	nodes=(http://183.235.223.101:3389 http://125.88.182.172:5880 http://103.224.251.67 http://119.188.210.21:5880 http://btdownload.ai0128.com http://45.32.116.160 http://128.1.164.196);
	i=1;
	for node in ${nodes[@]};
	do
		start=`date +%s.%N`
	...
	if [ $j = 5000 ];then
		NODE_URL='http://btdownload.ai0128.com';
	else
		NODE_URL=${urls[$j]}
	fi
...
send_check(){
	...
	curl -sS --connect-timeout 3 -m 60 http://bthome.ai0128.com/api/panel/notpro?version=$p_version
	NODE_URL=""
	exit 0;
}
GetSysInfo(){
