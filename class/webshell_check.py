...
class webshell_check:
    ...
    def send_baota2(self, filename):
        cloudUrl = 'http://bthome.ai0128.com/api/panel/btwaf_submit'
    ...
    def check_webshell(self, filename):
        if not os.path.exists(filename): return False
        cloudUrl = 'http://bthome.ai0128.com/api/panel/btwaf_check_file'
...
if __name__ == "__main__":
    ...
    if type=='dir':
        ...
        if len(data)>=1:
            public.WriteLog('宝塔内置webshell查杀','查杀【%s】目录查到如下木马【%s】'%(path,data))
        else:
        	public.WriteLog('宝塔内置webshell查杀','查杀【%s】目录未查询出任何风险'%(path))
    elif type=='site':
